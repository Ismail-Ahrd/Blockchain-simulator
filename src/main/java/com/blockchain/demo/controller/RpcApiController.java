...
public class RpcApiController {
    ...
    public ResponseEntity<Map<String,String>> addTransaction(@RequestBody Transaction transaction) {
        ...
        if(!blockchainService.verifyBalance(blockchain, transaction.getSender(),transaction.getAmount())){
            map.put("message", "you don't have enough NAM to send!");
            return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(map);
        }

        if(!generateKeyPair.verifySignature(transaction.getSender(), message, transaction.getSignature())){
            map.put("message", "verification failed , the public key is not yours !");
            return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(map);
        }
